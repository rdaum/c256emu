...
// handling windows, inputs, graphics context creation, etc.) (Requires:
// SDL 2.0. Prefer SDL 2.0.4+ for full feature support.)
...
//  compiling SDL with SDL_DISABLE_WINDOWS_IME.
...
// imgui, and hide them from your application based on those two flags. If you
// have multiple SDL events and some of them are not meant to be used by dear
// imgui, you may need to filter events based on their windowID field.
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
  ...
  io.MouseDown[2] =
      g_MousePressed[2] || (mouse_buttons & SDL_BUTTON(SDL_BUTTON_MIDDLE)) != 0;
  ...
  if (g_Window == focused_window) {
    ...
    // SDL_CaptureMouse both seems to severely mess up with that, so we retrieve
    // that position globally.
    int wx, wy;
...
#define MAP_BUTTON(NAV_NO, BUTTON_NO)                                          \
  {                                                                            \
    io.NavInputs[NAV_NO] =                                                     \
        (SDL_GameControllerGetButton(game_controller, BUTTON_NO) != 0) ? 1.0f  \
                                                                       : 0.0f; \
  }
